...
#'
#' @examples
ComponentPlot <- function(identity = "orig.ident", component = "DKCC", 
                          feature = "MALAT1", classification = "DKCC",
...
SankeyPlot <- function(simple = T,
                       ...
                       nephron.only = F){
  data <- data.frame()
  data <- reference %>% filter(Tier == classification)
  if (classification == "DKCC"){
    data <- bind_rows(data, GE$dkcc)
  }
  if (classification == "LineageID"){
    data <- bind_rows(data, GE$lineageid)
  }
  if (classification == "NephronID"){
    data <- bind_rows(data, GE$nephronid)
  }
  
  
  samples <- data$Identity %>% unique()
...
DotPlotCompare <- function(comp.data,
                           new.object = NULL,
                           show = "gene",
                           split.by = "orig.ident",
                           scaling = "log",
